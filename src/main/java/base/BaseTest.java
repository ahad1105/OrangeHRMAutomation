	...
	public void setupExtentReports() {
		if (extent == null) { // Ensure only one instance of ExtentReports
            sparkReporter = new ExtentSparkReporter(
                System.getProperty("user.dir") + File.separator + "reports" + File.separator + "OrangeHRMTestResults.html"
            );
            extent = new ExtentReports();
            extent.attachReporter(sparkReporter);

            sparkReporter.config().setTheme(Theme.STANDARD);
            sparkReporter.config().setDocumentTitle("OrangeHRM Automation Results");
            sparkReporter.config().setReportName("OrangeHRM automation");

            extent.setSystemInfo("Enviroment", "OrangeHRM Pre-Prod");
            extent.setSystemInfo("Username", "Khanaha");
    }
	}
